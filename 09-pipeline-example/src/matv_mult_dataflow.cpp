...
void load_vecB(int* b_local, const int* b, const int row) {
    // load vector b into local RAM
    I_load_b:
    for (int i = 0; i < BLOCK_DIM_SIZE; i++) {
        #pragma HLS loop_tripcount max=BLOCK_DIM_SIZE
        b_local[i] = *(b + i*BLOCK_DIM_SIZE + row );
    }
}
...
void store_result(int* c, int* c_local, int row) {
    // store vector c into global RAM
    I_store_c:
    for (int i = 0; i < BLOCK_DIM_SIZE; i++) {
        #pragma HLS loop_tripcount max=BLOCK_DIM_SIZE
        *(c + i*BLOCK_DIM_SIZE + row) = c_local[i];
    }
}
